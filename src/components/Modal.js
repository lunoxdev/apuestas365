      ...
      <div className="flex items-center bg-gray-800 bg-opacity-70 justify-center min-h-screen">
        <div className="bg-primary rounded-2xl shadow-black shadow-2xl w-5/6">
          <div className="px-4 py-2">
            <h2 className="text-3xl font-bold">{sportsbook.name}</h2>
          </div>
          <div className="flex justify-center">
            <div
              className={`px-4 py-2 ${
                activeTab === 1
                  ? "text-white bg-gray-800 first:border-t-4 border-blue-700"
                  : ""
              } cursor-pointer`}
            ...
            <div
              className={`px-4 py-2 ${
                activeTab === 2
                  ? "text-white bg-gray-800 border-t-4 border-blue-700"
                  : ""
              } cursor-pointer`}
          ...
          <div className="px-4 py-4">
            {activeTab === 1 && (
              <div>
                <div className=" text-xs p-2 mb-4 text-justify">
                  <p>
                ...
                {/* // Información */}
                <div className="grid grid-cols-3 lg:grid-cols-6 gap-6 p-3 text-center mb-5">
                  {/* Casino */}
                  ...
                  <div className="flex justify-center mt-3">
                    <ul className="grid grid-cols-3 gap-6">
                      {sportsbook.metoPagos.map((metodoPago, index) => (
                        <div key={index}>
                          <li className="text-xs">{metodoPago}</li>
                        </div>
                      ))}
                    </ul>
                  </div>
          ...
          <div className=" mb-4 px-4 py-2 flex lg:justify-center">
            <button
              className="px-4 py-2 w-full lg:w-96 bg-btn text-white hover:opacity-80 rounded-md font-bold"
              onClick={onClose}
            >
              Cerrar
            </button>
